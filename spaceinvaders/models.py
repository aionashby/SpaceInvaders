...
class Ship(GImage):
    ...
    """
    def __init__(self, x=400, y=SHIP_BOTTOM, width= SHIP_WIDTH, \
    height = SHIP_HEIGHT, source= 'ship.png'):
        """
        Initalizes a new Ship object.
        """
        super().__init__(x=x,y=y,width=width,height=height,source=source)

    def pbolt_collides(self, bolt):
        """
        Returns True if a bolt collides with the ship, returns False if otherwise.
        """
...
class Alien(GImage):
    ...
    LIST MORE ATTRIBUTES (AND THEIR INVARIANTS) HERE IF NECESSARY
    """
    def __init__(self, x=400, y=350, width= ALIEN_WIDTH, \
    ...
    def collides(self, bolt):
        """
        Returns True if a bolt collides with an alien.
    ...
    def line_collides(self, defense):
        """
        Returns True if the defense line collides with an alien.

        Parameter Defense:
        Precondition:
        """
        if self.contains((GAME_WIDTH, DEFENSE_LINE)):
            return True
        else:
            return False
...
class Bolt(GRectangle):
    ...
    def __init__(self, x, y, velocity, fillcolor = 'pink'):
        """
        Initalizes a new Bolt object.
        """
    ...
    def move(self):
        """
        Increases the bolts y position by the velocity.
        """
